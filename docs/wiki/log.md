...
## [2026-08-17] fix | 1500 tokens não cabiam o turno que FECHA a entrevista
- `decisoes/entrevista-da-mestre.md` — entrevista de 15 turnos com conteúdo real (Eiai Solutions) travou no "sim pode fechar": `stop=max_tokens`, duas vezes. `concluir_entrevista` emite os **14 campos de uma vez** e os blocos `thinking` consomem o **mesmo** orçamento de saída. Com `_MAX_TOKENS=1500` o `tool_use` era truncado, o bloco truncado descartado (senão vira órfão e envenena a conversa) e não sobrava nada.
- **Quanto MELHOR a entrevista, mais certo era travar no fim** — o oposto do que se espera de um limite de segurança.
- É um teto **por chamada**, não uma meta: só se paga o que for gerado, e as perguntas medem 150-300 tokens. O freio real é `MAX_TOKENS_SAIDA`, por entrevista. Subiu para 8000.
- **A lição:** este era o achado #1 da varredura adversarial, que eu confirmei e corrigi **pela metade** — tratei o SINTOMA (não gravar `tool_use` órfão) e não a CAUSA (o teto pequeno). Trocar um 502 permanente por um bloqueio permanente é melhor, mas continua impossível de concluir. Quando a análise aponta a causa, corrigir só a consequência deixa o usuário igualmente parado.
- Junto: bloco só de `thinking` também não conta como resposta; e repetir a mensagem depois da falha não a duplica mais no histórico (o estado volta EXATAMENTE ao ponto anterior).
- **Operacional:** o deploy automático do EasyPanel parou de disparar depois de `6a090cc` — a fonte no servidor ficou em 13:07 UTC. Não construí imagem à mão por fora do pipeline: cirurgia em swarm fora do EasyPanel já reverteu o `DATABASE_URL` neste projeto.

## [2026-08-17] decisao | Tetos da entrevista configuráveis, e desligados por padrão
- `decisoes/entrevista-da-mestre.md` (**atualizado**) — decisão do Luiz: *"a entrevista não precisa ter limite por enquanto, depois eu defino o limite no painel admin"*. Em vez de apagar os tetos, viraram configuração com default `0` = sem limite. Quando o painel existir, ele só escreve o número — não reescreve a lógica.
- **Implicação registrada:** a rota é pública e ANÔNIMA e gasta a nossa chave Anthropic. Com tudo em `0`, o único freio de pé é o rate limit de 20/min por IP — segura abuso automatizado, não uso lento e contínuo.
- **A pegadinha:** `MAX_TURNOS` também segurava, de carona, o laço interno de `avancar`. `pause_turn`/tool_use podem se repetir indefinidamente dentro de um único turno; com os tetos em `0` o request nunca responderia. Entrou `MAX_VOLTAS_NO_LOOP = 24`, **não configurável** — limite de engenharia, não de produto. Mutation-testado: sem ele, o laço consome o roteiro inteiro e continua pedindo.
- **Lição geral: antes de desligar um limite, procure quem mais dependia dele sem dizer.**

## [2026-08-17] fix | Fases 0 e 1 do multi-agente: o painel lista por nome, e cada canal lê o SEU agente
- Plano completo (6 fases, 10 decisões, 90 pontos de acoplamento): artefato publicado nesta sessão. O pedido tem **duas coisas coladas** — (A) endereçar o agente por `id` em vez de `(location_id, channel)`, que é refactor sem migration; e (B) duas contas no mesmo canal, que é arquitetura. **A é pré-requisito de B**, e entrega valor sozinha porque WhatsApp + Telegram já coexistem hoje.
- **A regra que rege o resto:** não derrubar `UniqueConstraint("location_id","channel")` antes de todo call site endereçar por conta. Hoje essa trava é a única coisa que impede os ~30 `.first()` de virarem loteria; tirá-la cedo troca um `IntegrityError` alto e visível por **roteamento silenciosamente errado**.
- **A classe de bug que domina esta área: o LATENTE.** Com um agente por instância — o caso de produção hoje (verificado: 1 tenant, 1 agente) — `.first()` sem filtro de canal acerta por sorte. Ativa no minuto em que o segundo agente é criado, que é justamente o que o painel passou a permitir. E ativa **sem erro no log**: o pior era pausar o agente do Telegram **desligar a IA do WhatsApp**.
- Três lacunas fecharam depois: `zapi_receiver` é uma **segunda entrada de WhatsApp** fora do pipeline compartilhado, com cópia própria do kill-switch e do debounce — o gêmeo foi corrigido num caminho e ficou de pé no outro; `delete_agent_by_channel` era `.filter(...).delete()`, delete de CONJUNTO, no lugar mais irreversível possível; e `restore_version` achava o agente por canal, com `agent_id` já gravado em toda snapshot e **nunca lido de volta** — o prompt É o agente, restaurar no errado troca a persona de quem atende.
- **Lição sobre teste, não sobre código:** a primeira versão do teste de `restore_version` **passava sem a correção**. Com um agente por canal na fixture, o fallback acerta por sorte — que é exatamente o bug que se queria pegar. Só divergindo `agent_id` de `channel` na versão gravada o teste mede quem manda. **Num bug latente, a fixture precisa conter o futuro que o torna ativo**, senão o teste testa a sorte.

## [2026-08-17] add | Múltiplos agentes por instância — metade A no ar, metade B planejada
- `decisoes/multiplos-agentes-por-instancia.md` (**novo**) e `decisoes/multi-agente-plano-completo.md` (**novo**) — o pedido tinha **duas metades de custo muito diferente** embaralhadas na mesma frase: listar agentes por nome (painel, sem decisão de produto) e duas contas no mesmo canal (arquitetura, 7 decisões). Mapeamento com 4 leitores paralelos: **92 pontos de acoplamento**.
- **Correção de premissa:** o `CLAUDE.md` dizia "6 agentes em produção" — não é mais verdade. O banco real tem **1 tenant, 1 agente**; os 6 eram do Postgres do VPS, antes do Supabase. O próprio plano mandava rodar essa contagem como primeiro passo, e ela barateou muito a migração de dados da metade B.
- **Metade A entregue:** a aba "Agente IA" lista os agentes por nome/canal/estado e clicar abre a config daquele agente. Ficou pequena porque quase tudo já existia e estava sendo descartado — `/list` já devolvia `id`/`name`/`linked_to_channel` e o modal já sabia trocar de agente via `switchChannel`; faltava alguém dizer QUAL.
- **Os bugs latentes que vinham junto:** 9 de 33 queries de `AIAgent` não filtravam canal. Com um agente só o `.first()` acerta por sorte. O pior: `ai_is_enabled` lia `is_active` de um agente arbitrário — **pausar o agente do Telegram desligava a IA do WhatsApp**, sem erro no log.
- **O padrão da metade B, registrado antes de começar:** quase toda falha lá retorna **200 e sucesso aparente** (agente errado responde, resposta sai pelo número errado marcada `delivered`, mídia 404, memória contaminada). Argumento para investir em métrica de coerência ANTES de mexer no roteamento.
- A mina mais feia mapeada: `admin/waha.py:180` grava `t.waha_session = session` — conectar um 2º número **apaga a credencial do 1º**, que segue rodando no WAHA mandando webhook órfão.

## [2026-08-17] fix | XSS armazenado no painel, achado pela revisão da própria correção
- `decisoes/multiplos-agentes-por-instancia.md` (**atualizado**) — a revisão adversarial sobre os commits da metade A achou **8 problemas, 1 grave e meu**: o `name` do agente ia para `title="…"` escapando só `<`. Aspa dupla fecha o atributo e o resto vira **atributo do `<button>`** — `onmouseover=` executando na mesma página onde as chaves de API são renderizadas. Sem CSP no projeto.
- **Três regras que saíram disso:** (1) escapar `<` não protege em contexto de atributo — o perigo é a aspa, e o ataque não abre tag nenhuma; (2) **`_escapeHtml` deste projeto não serve para atributo** (é `textContent→innerHTML`, não escapa aspas) — trocar por ele só deslocaria o buraco; (3) `agent_validators.py` só limita o TAMANHO do `name`, enquanto `QualificationField.key` rejeita aspas explicitamente — e a Mestre escreve nesse campo a partir da entrevista pública.
- **A correção não foi escapar melhor:** a lista passou a ser montada por DOM (`setAttribute`/`textContent`/listener), imune aos três contextos de uma vez. Verificado em navegador com a carga do achado. **Regra: nada de dado do banco dentro de template string que vira `innerHTML`.**
- Outros sete: snapshot do `restore` nascia órfã; clicar no WhatsApp não buscava e abria com o `dataset` do carregamento da página; lista sem carimbo de requisição (resposta de uma instância pintando no modal de outra); DELETE virou erro para canal sem registro, deixando aba fantasma; tester nunca recebeu `channel` — **a mensagem do meu commit afirmava que a tela mandava, e não mandava**; `_agente_da_tela` não resolvia alias.
- **A lição de método:** revisar a própria correção pagou de novo. Foi a quarta vez no dia que a passada adversarial achou algo real em código que eu já tinha dado por pronto e deployado.

## [2026-08-17] fix | O teste que não exercita o ponto de entrada real não testa nada
- Migration 034 (`channel_accounts`) foi ao ar e a revisão adversarial achou 3 defeitos — os dois graves **reconferidos à mão contra o banco de produção** antes de qualquer correção.
- **A FK nunca entrou em produção.** O modelo declara `ForeignKey("channel_accounts.id")`; a migration adicionou a coluna como INTEGER puro. Como **`create_all` não altera tabela que já existe**, em produção a única via era a migration — e a constraint ficou de fora. Num banco nascido do zero o `create_all` cria a tabela inteira já com a FK. **Mesmo código, dois schemas.** Medido: produção tinha só a FK de `location_id`.
- O precedente correto era da `030`, mas com uma armadilha: lá o `create_foreign_key` está **dentro** do `if not _column_exists`. Com a coluna já existindo, ele nunca roda. Copiar o precedente sem ler teria reproduzido o bug.
- **A causa raiz não é nenhum dos três achados — é o ponto cego que os produziu.** Os testes montavam o schema com `create_all` e chamavam só `_backfill()`. O **`upgrade()`, que é o que produção executa, nunca era exercitado**: 678 testes passavam sem tocar no DDL real. Um teste que monta o mundo por um caminho e verifica outro não testa o que se acha que testa.
- Para rodar o alembic de verdade no teste foi preciso descobrir que `alembic/env.py:42` monta o engine a partir de `data.database.SQLALCHEMY_DATABASE_URL` e **ignora o `sqlalchemy.url` do Config** — sem apontar o módulo, o teste roda contra o banco de dev e "passa".
- Terceiro achado, da mesma família do `is_active` que já tinha mordido: **`created_at`/`updated_at` nasceram NULL**, porque o backfill insere por SQL cru e não passa pelos defaults do ORM. E cópia de coluna SEM tamanho (tenant) para coluna COM tamanho (conta): SQLite ignora `VARCHAR(n)`, Postgres recusa — e migration que falha derruba o boot para todos.
- Limite registrado na própria fixture: o SQLite recusa dropar coluna referenciada por FK, então não dá para reproduzir ali a forma exata de produção. A criação da constraint é verificada por leitura do código e foi conferida à mão no servidor.

## [2026-08-17] feat | Fase 3a: a mensagem sabe qual conta a recebeu (WAHA)
- O envelope do WAHA **sempre** trouxe `session` na raiz — a identidade da conta que recebeu — e ela era descartada na porta. Sem esse dado, `(location_id, channel)` é a única chave e o segundo número seria indistinguível do primeiro.
- `ParsedMessage.account_ref` atravessa; `services/channel_accounts.resolver()` traduz a referência do provedor para a linha em `channel_accounts`; `ai_service` busca o agente **pela conta** antes de cair no canal. A chave do cache de engine ganhou a conta — sem isso o motor do primeiro número serviria o segundo.
- **Fallback é regra, não cortesia.** Três caminhos caem nele e continuam funcionando: adapter que ainda não informa a conta (Z-API, Telegram), conta sem agente próprio, e duas contas sem referência — onde **sortear seria pior que não responder**, porque a persona errada atenderia sem erro no log. É o que permite ligar a fase sem big-bang.
- `uq_ai_agent_location_channel` continua de pé: é ela que impede os `.first()` restantes de virarem loteria enquanto o roteamento não endereça tudo por conta.

### Dois erros meus no caminho, os dois da mesma família: o teste que mede nada
1. **`ai_service` liga o `SessionLocal` no import do módulo.** Patchear só `data.database` no teste o deixava consultando o banco de DEV — e o teste passava medindo nada. Só apareceu porque a suíte completa quebrou; isolado, passava.
2. **O teste de precedência não distinguia conta de canal.** Os dois caminhos achavam o mesmo agente, então remover a busca por conta o mantinha verde. Refeito divergindo o canal do agente da conta a que ele pertence — estado artificial de propósito, porque `uq_ai_agent_location_channel` impede o cenário real.
- **O padrão, agora pela terceira vez no mesmo dia** (aconteceu no `restore_version` e na migration 034): *quando o cenário que ativa o bug ainda não é alcançável, a fixture precisa construí-lo divergindo algum eixo — senão o teste mede a sorte, igual ao código.* E a mutação é o que revela: teste que continua verde com a correção removida não é teste.

## [2026-08-17] fix | A conta de canal não tinha produtor — consumidor sem produtor, de novo
- A revisão adversarial da fase 3a achou: o único escritor de `channel_accounts` era o backfill da migration 034, que roda **uma vez**. Toda instância conectada depois do deploy ficava sem conta.
- **O roteamento não quebrava** — o fallback por canal segurava. O estrago era outro e mais difícil de perceber: o adapter do WAHA *sempre* preenche `account_ref` (a sessão cai em `location_id` quando `waha_session` é nulo), então o resolver entrava no ramo da referência, não achava nada e logava WARNING **por turno, para sempre**. O alarme criado para denunciar persona errada passava a disparar no estado NORMAL de toda instância nova, enterrando o próprio sinal.
- **É o mesmo cheiro que eu já tinha identificado nas portas do wizard e não reconheci ao escrever isto: CONSUMIDOR SEM PRODUTOR.** Campo lido e nunca escrito = falta metade do fluxo. Vale como checagem fixa: ao criar tabela nova, perguntar *quem escreve* antes de escrever quem lê.
...
## [2026-08-18] fix | Um `await` no lugar errado quebra atomicidade que ninguém declarou
- Revisão adversarial da 3b: 5 achados, 4 confirmados. O grave é meu e vale como padrão.
- **Eu pus a resolução da conta ENTRE o append no buffer de debounce e o `create_task`.** Esse trecho era síncrono, portanto **atômico**: todo item que entrava no buffer ganhava uma task. Com uma exceção subindo pelo `await`, o `create_task` não roda e a mensagem fica **órfã** — invisível aos dois limpadores (que derivam as chaves de `_ai_pending_tasks`, nunca do buffer), colada no próximo turno daquele contato, e acumulando até o hard cap de 2000, quando o agendamento da IA morre para **todos os contatos de todos os tenants** até reiniciar o processo.
- **A lição: introduzir `await` no meio de uma sequência síncrona quebra uma invariante que ninguém escreveu.** Atomicidade por ausência de `await` é real e é invisível — não há comentário, não há teste, e o `except` genérico em volta transforma a quebra em silêncio. Ficou teste que falha se algum `await` voltar para aquele trecho.
- **A assimetria era o bug**: `_sincronizar_sync` engolia exceção de propósito ("não pode derrubar a conexão do canal"); `_resolver_sync` não, apesar de o contrato do módulo ser "não sei dizer → None → fallback". Escrever a guarda num gêmeo e não no outro é pior que não escrever em nenhum.
- E **`SessionLocal()` estava fora do `try`** nos dois: pool esgotado ou conexão recusada estouram ali, antes do primeiro SELECT. Quem achou foi o teste que escrevi para a própria correção — ela não cobria o caso que tentava cobrir.
- **O produtor criava a conta e não ligava o agente.** O backfill da migration ligava; o de runtime não. Instância nova ganhava conta e o roteamento por conta ficava **inerte**, sempre no fallback. Consumidor sem produtor tem irmão: produtor pela metade.
- O teste do `NameError` do Python 3.11 fixava **um símbolo num módulo** — não fecha a classe. Agora varre todo módulo do projeto carregado com `typing.get_type_hints`.

## [2026-08-18] add | Medir antes de melhorar: o produto mandava o agente errar
- `decisoes/medir-antes-de-melhorar.md` e `decisoes/mestre-melhoria-plano.md` (**novos**) — pedido era "deixar a Mestre testadora extremamente boa". Painel de 3 propostas + 3 juízes concluiu que **primeiro o produto precisa parar de mandar o agente errar**.
- **Três defeitos silenciosos, corrigidos e no ar:** (1) `master_prompt.py:293` obrigava a Mestre a escrever "transfira quando o lead estiver pronto pra fechar" — e no SDK transferir **pausa** a conversa; isso explica o "escala tudo" que atribuíamos a prompt fraco, **era instrução explícita**; (2) `_claude_tool_dispatch` devolvia `"ok"` incondicional e o guard descartava depois — o lead ouvia "um especialista vai te chamar" e **o CRM ficava vazio**, com a conversa salva parecendo um fechamento perfeito; (3) a `description` do campo escrita pela Mestre era descartada do `input_schema`.
- **A sonda** (`scripts/sonda_agente.py`) roda o `ClaudeAgentEngine` real contra roteiros e reporta **qual ferramenta** o agente chamou. Mede ação, não texto. `--historico <id>` mede versão antiga: é assim que se prova uma melhoria.
- **O primeiro uso derrubou minha própria conclusão:** prompt corrigido 6/6, prompt ANTERIOR **também 6/6**. A correção não produziu diferença mensurável. Continua certa, mas eu teria anunciado vitória não ganha. **Construir o instrumento antes de comemorar** — sem ele, três correções plausíveis viram três alegações não verificadas.
- **Consequência honesta:** roteiro que passa antes E depois estabelece piso, não discrimina. Casos que separam bom de ótimo precisam sair de conversa real.
- Estado do agente do Luiz: `qualification_enabled=False`, 3 campos todos `auto` → sem campo de coleta, `build_tool_specs` (corretamente) não oferece `register_qualified_lead`, e a única ação que a Ellen conhece é escalar. Ele vai cadastrar os campos.
- **Em aberto:** o pedido original. A Mestre roda em OpenRouter `gpt-4o` com 280 palavras que não mencionam ferramenta nem qualificação, enquanto o agente é Anthropic tool-use.
