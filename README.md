# MilloChat — Integração Z-API & GoHighLevel
...
- **Outbound Webhook (GHL → Z-API)**: Captura as mensagens enviadas no MilloChat (GHL) enviando via WhatsApp (Z-API). Suporta anexos.
...
Sempre que a sua agência vender a automação *MilloChat* para uma nova Imobiliária/Empresa, execute os seguintes passos:

1. Gere uma nova **Client Key** e Secret para o app "MilloChat" lá no dashboard de desenvolvedor do GHL (página Marketplace > MyApps). Isso vincula o aplicativo de forma única a essa instalação.
...
```
Exemplo Real:
`https://millochat.com/webhook/zapi/inbound/FkO3l88oXyV3i`
